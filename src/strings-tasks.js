...
function isPalindrome(str) {
  const string = str.replaceAll(/[,!.? ' ']/g, '').toLowerCase();
  const stringLength = string.length;
  const part1 = string.slice(0, Math.floor(stringLength / 2));
  const part2 = string
    .slice(Math.ceil(stringLength / 2))
    .split('')
    .reverse()
    .join('');
...
function reverseWords(str) {
  ...
  const newArray = array.map((el) => el.split('').reverse().join(''));
